...
static int ftp_process_request(struct conn *conn, char *buf)
{
    ...
    case CMD_PORT:
	{
		...
		pasvremote.sin_addr.s_addr = ((uint32_t) portcom[3] << 24) | ((uint32_t) portcom[2] << 16) | ((uint32_t) portcom[1] << 8) | ((uint32_t) portcom[0]);
	...
	do {
	    ...
	    do_send_reply(conn->sockfd, "227 Entering passive mode (%d,%d,%d,%d,%d,%d)\r\n",
			...
			(uint8_t)(ipaddr.addr >> 8),
			(uint8_t)(ipaddr.addr >> 16),
			(uint8_t)(ipaddr.addr >> 24),
                          dig1, dig2);
