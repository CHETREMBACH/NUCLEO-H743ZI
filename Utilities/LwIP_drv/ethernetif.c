...
#define TIME_WAITING_FOR_INPUT                 ( 10 )
...
/* Notify Task to signal incoming packets */
TaskHandle_t  EthIfTaskNotify = NULL;
...
static void low_level_init(struct netif *netif)
{
  ...
  xTaskCreate(ethernetif_input, (const char*)"EthIf", INTERFACE_THREAD_STACK_SIZE, netif, TCPIP_THREAD_PRIO, &EthIfTaskNotify);
...
void ethernetif_input(void * pvParameters)
{
  ...
  for( ;; )
  {
    if (ulTaskNotifyTake(pdTRUE, TIME_WAITING_FOR_INPUT) != 0)
    {
      do
      {   
        LOCK_TCPIP_CORE();
...
void HAL_ETH_RxCpltCallback(ETH_HandleTypeDef *heth)
{
  ...
  UNUSED(heth);	
  if (EthIfTaskNotify != NULL)
  {
    vTaskNotifyGiveFromISR(EthIfTaskNotify, &xHigherPriorityTaskWoken);		
  }
...
/**
  ...
  * @retval None
  */
void ethernet_link_thread(void *arg)
{
  ETH_MACConfigTypeDef MACConf;
  int32_t PHYLinkState;
